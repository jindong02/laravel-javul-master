                ...
                <div class="panel panel-grey panel-default">
                    <div class="panel-heading">
                        @if(empty($categoryObj))
                            <h4>Create Job Skill</h4>
                        @else
                            <h4>Update Job Skill</h4>
                        @endif
                    </div>
                                ...
                                <div class="row form-group">
                                    <div class="col-sm-4 form-group {{ $errors->has('skill_name') ? ' has-error' : '' }}">
                                        ...
                                        <div class="input-icon right">
                                            <i class="fa"></i>
                                            <input type="text" name="skill_name" value="{{ (!empty($skillObj))? $skillObj->skill_name : old('skill_name')}}"
                                                   class="form-control"
                                    ...
                                    <div class="col-sm-4 form-group {{ $errors->has('parent_id') ? ' has-error' : '' }}">
                                        ...
                                        <div class="input-icon right">
                                            ...
                                            <select name="parent_id" id="parent_id" class="form-control">
                                                @if(count($parent_skills) > 0)
                                                    <option value=""></option>
                                                    @foreach($parent_skills as $id=>$p_skill)
                                                        <option value="{{$id}}" @if(!empty($skillObj) && $skillObj->parent_id == $id)
                                                        selected="selected" @endif>{{$p_skill}}</option>
                                                    @endforeach
                                                @endif
                                            </select>
                                        </div>
                                ...
                                <div class="row form-group">
                                    <div class="col-sm-12 ">
                                        <button id="create_category" type="submit"  class="btn black-btn">
                                            @if(!empty($skillObj))
                                                <span class="glyphicon glyphicon-edit"></span> Update Skill
                                            @else
                                                <i class="fa fa-plus plus"></i> <span class="plus_text">Create Skill</span>
                                            @endif
                                        </button>
...
@section('page-scripts')
    ...
    <script src="{!! url('assets/js/admin/skills.js') !!}"></script>
@endsection